...
/**
 * Client-mode settings card — rendered only when the kernel runs as a local
 * client instance (the settings page gates server-side on isClientMode()).
 * Four sub-blocks in one card: read-only runtime status, local agent CLI
 * detection (/api/client/agents), brain selection, and the execution
 * backend — the latter two saved via /api/client/config. Deliberately a
 * status panel, not a console.
 */
...
/** Native <select> styled like ui/input — ui/select (Base UI) doesn't render
 *  a native select, which the e2e specs drive via selectOption(). */
const selectClass =
...
export function ClientSection() {
  ...
  return (
    <Card>
      ...
      <CardContent>
        {loadError && (
          <p className="text-xs text-red-500 mb-4">{t("loadFailed")}: {loadError}</p>
        )}
