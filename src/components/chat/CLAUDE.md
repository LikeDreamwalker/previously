...
2. `ChatMessage.buildStream()` classifies each part in a single pass:
   ...
   - `data-evolution` → its OWN `EvolutionCard` item at the position the chunks arrive (between the housekeeping context and strands phases); while running it streams the Previously Agent's live thinking line, the terminal chunk carries the summary / mutations diff / note / error / partial flag, plus the v1.0 calibration details (fitness `triggers` + net scores, the Phase-1 `direction` verdict, Phase-2 `playbooks`)
