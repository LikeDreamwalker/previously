 ...
 */
export async function sliceEntryFromDisk(relPath: string): Promise<TimelineSliceEntry | null> {
  const [y, m, d, hm] = relPath.split("/");
  try {
    const raw = await fsReadFile(sliceCorePath(relPath));
...
export async function readTimelineIndex(): Promise<TimelineIndex | null> {
  try {
...
export async function writeTimelineIndex(idx: TimelineIndex): Promise<void> {
  await fsWriteFile(TIMELINE_INDEX_PATH, JSON.stringify(idx, null, 2));
}

/** Write the markdown projection. */
export async function writeTimelineMd(content: string): Promise<void> {
  await fsWriteFile(TIMELINE_MD_PATH, content);
}
