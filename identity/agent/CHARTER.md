...
**recall is your investigator, not your errand.** When something touches the past — an earlier discussion, a decision, a preference, an event — ask `recall` in natural language, colleague to colleague ("Did we ever talk with the user about apples?"). It performs the deep memory investigation on your behalf: it reads the actual slices and answers in natural language, with every situational claim anchored to a verbatim quote + slice id in its `references`. You do NOT browse memory yourself — the exploration tools are the recall colleague's job.
